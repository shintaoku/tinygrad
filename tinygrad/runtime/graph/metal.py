...
class MetalGraph(GraphRunner):
  ...
  def compile_host_launcher(self, input_rawbuffers: list[Buffer]):
    ...
    cmd_buf_arg = UOp(Ops.DEFINE_GLOBAL, dtypes.uint64, arg=10)
    encoder_arg = UOp(Ops.DEFINE_GLOBAL, dtypes.uint64, arg=11)
    ...
    exec_icb = objc_msg(encoder_arg, "executeCommandsInBuffer:withRange:", 
                       icb_arg, 
                       UOp(Ops.CUSTOM, dtypes.void, (range_start_arg, range_len_arg), arg="NSMakeRange"),
                       )
    
    graph = UOp(Ops.SINK, dtypes.void, (loop, exec_icb))
  ...
  def _compile_and_load(self, src: str):
    # 最初の "{" を探して、それ以降（ボディ）を取り出す
    body_start = src.find("{")
    # 最後の "}" を探す
    body_end = src.rfind("}")
    
    if body_start == -1 or body_end == -1:
        raise RuntimeError("Failed to parse generated C code")
    ...
    void* launch_metal_graph(
        id<MTLIndirectCommandBuffer> data0,
        id<MTLBuffer>* data1,
        int* data2,
        int* data3,
        int data4,
        id<MTLResource>* data5,
        int data6,
        id<MTLCommandQueue> data7,
        int data8,
        int data9,
        void* data10_dummy, 
        void* data11_dummy
    ) {{
        id<MTLCommandBuffer> commandBuffer = [data7 commandBuffer];
        id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];
        
        id<MTLCommandBuffer> data10 = commandBuffer;
        id<MTLComputeCommandEncoder> data11 = encoder;
        
        {body}
        
        [encoder endEncoding];
        
        return (void*)CFBridgingRetain(commandBuffer);
    }}
    """
    ...
    try:
        ...
        lib.launch_metal_graph.restype = ctypes.c_void_p
        return lib
    finally:
        pass
  ...
  def __call__(self, input_rawbuffers: list[Buffer], var_vals: dict[str, int], wait=False) -> float|None:
    ...
    ptr = self.lib.launch_metal_graph(
        self.icb,
    ...
    command_buffer = metal.MTLCommandBuffer(ptr)
